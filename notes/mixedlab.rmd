...
usedpkgs <- sort(c("lme4","nlme","glmmADMB","R2jags",
...
library("MCMCglmm") ## Bayesian GLMMs
...
## reorder individuals by difference
...
We get an error: if we fit this with

```{r lmefit1,eval=FALSE}
lme(stmass~mnth*roostsitu, random= ~1|subject/mnth,data=dataf)
```
it lets us do it, silently (but runs into trouble farther down the road). We *can* actually do this (`lmerControl(check.nobs.vs.nlev="ignore")`; see `?lmerControl` for details), and it will actually work, but it will pick an arbitrary division between the residual variance and the `month:subject` variance.
...
Basic diagnostic plots: fitted vs. residuals, coloured according to the `roostsitu` variable:
```{r lmediag}
...
We can also do this with `tidy::augment()` and `stat_qq()` (although there are also some some limitations - `colour` argument doesn't actually work...)
```{r ggqq,fig.keep="none"}
...
(It's usually a good idea to drop the intercept from coefficient plots: the next release of `dwplot` will have a `drop_intercept` parameter that will do this automatically.) For more complicated coefficient plots, you can use the combination of the `broom` package to get the parameters in a useful form and `dwplot` to draw the plot.
...
Also note that `nlme::lme` gets denominator df calculations wrong for some random-slopes models. If you want to try `lme`'s denominator df calculations without actually fitting the model:
```{r calcdendf,}
source("R/calcDenDF.R")
calcDenDF(~roostsitu*mnth,"subject",dataf)
```
...
```{r pboot,cache=TRUE,warning=FALSE}
...
Unfortunately computing confidence intervals for the predictions is a little tricky: again, there is some code on the [GLMM faq](http://tinyurl.com/glmmfaq) for this (also see below under `MCMCglmm`).
...
ggplot(dataf,aes(x=pred1,y=mpred1.fit))+geom_point()+
  ...
  labs(x="lmer prediction",y="MCMCglmm prediction")
```
